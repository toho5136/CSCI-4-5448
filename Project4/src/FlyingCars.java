...
public class FlyingCars extends Vehicles {
	private int fluid; //EC unique attribute
	public int getFluid() {
		return this.fluid;
	}
	public void setFluid(int rang) {
		fluid = rang;
	}
	public FlyingCars(String identity){
		setID(identity);
		setCarType("FlyingCars");
		setSalesBonus(500.0); //price copied from Cars
		setRepairBonus(50.0); //copied from cars
		setWashBonus(10.0); //copied from cars
		setFluid(ThreadLocalRandom.current().nextInt(60, 400+1)); //Setting initially as 60-400
		...
		if (conditionRoll == 1){
			...
			setFluid(getFluid() + 100); //Increases by 100 if Like New
		}
